...
function createComment(url, request) {
  ...
  if (requestComment && requestComment.body &&
      requestComment.username && requestComment.articleId && database.users[requestComment.username]) &&
...
function deleteComment(url, request) {
    ...
    let savedArticle = database.articles[1]
    const response = {};

    if (savedComment && id) {
      database.comments[id] = null;
      savedArticle.commentIds.forEach((x,i) => {
        if (id === x) {
          savedArticle.commentIds.splice(i, 1);
...
function upvoteComment(url, request) {
  ...
  if (!savedComment || !database.users[username] || !id) {
    response.status = 400;
  } else if (!savedComment.upvotedBy.includes(username)) {
...
function downvoteComment(url, request) {
  ...
  if (!savedComment || !database.users[username] || !id) {
    response.status = 400;
  } else if (!savedComment.downvotedBy.includes(username)) {
